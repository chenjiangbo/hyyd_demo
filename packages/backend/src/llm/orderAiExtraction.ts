...
export const ORDER_AI_PROMPT_RULES = [
  ...
  '2. 典型触发场景：',
  '   - 服务人员在企微/微信中正式向客户发送取消确认通知（例如：“如刚才电话沟通，此次就医服务取消”、“本次服务到此结束……群就解散了”等）；',
  '   - 客户在聊天或电话录音中明确提出退单取消（例如：“这次不看了，帮我退单/取消吧”、“家里有事不去看了，服务取消吧”），且服务人员已应答确认；',
  '   - 双方在通话录音中明确沟通并达成一致意见取消本次就医服务。',
  '3. 严禁误判禁令（防误判原则）：',
  '   - 严禁将客户犹豫口吻、假设性询问、暂缓商量或改期看诊误判为取消（例如：“如果挂不上我就取消”、“可能去不了了看看情况”、“能不能换个时间再看”等绝对不是取消）；',
  '   - 严禁将单次退号/挂号未成但后续继续沟通其他医院/医生/时段的行为误判为整单取消；',
  '   - 只有百分之百确认整单就医服务彻底终止取消时才可输出 service_cancelled，证据必须引用载明取消事实的具体消息#ID或通话#ID及原文短句。',
