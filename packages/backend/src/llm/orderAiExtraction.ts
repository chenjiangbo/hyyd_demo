...
export const ORDER_AI_PROMPT_RULES = [
  ...
  '金额、诊断、ICD10 必须 requires_confirmation=true。医院、科室、医生只输出名称，不输出任何内部字典 ID。专家级别(expert_level)是医生的档案属性由医生主档联动带出，若已有明确具体医生，不要单独提取专家级别；仅未提及具体医生时才可提取标准枚举之一（知名专家、主任医师、副主任医师、主治医师、住院医师），绝不得包含“教授/博士”等非标教学称谓。',
