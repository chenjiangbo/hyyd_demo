...
> ⚠️ **2026-06-26 状态更正（先读这段——本文部分内容已过时）**
> - **后端已迁到阿里云 ECS `47.95.14.233`，端口 `9093`**（WS `/ws`、HTTP、插件自动更新都在此）。本文 §三「后端跑在 Mac、监听 0.0.0.0、Mac 内网 IP 192.168.99.165:13000」是早期本地开发期写法，已过时；MinIO 同在该 ECS。本地开发仍可用 13000。
> - **截图 OCR 已换成 RapidOCR（PP-OCRv5 / ONNX）**，不再用 Windows.Media.Ocr（附录 A5 已过时）；以 [`采集管道_实现细节.md`](采集管道_实现细节.md) 为当前实现。
> - **消息结构化仍在 sidecar(C#)**：附录 C「直接消费 sidecar messages[]」与 [`变更说明_结构化移至后端.md`](变更说明_结构化移至后端.md)「移到后端」都已被回退/取代，以 [`采集管道_实现细节.md`](采集管道_实现细节.md)（2026-06-16）为准——sidecar 出结构化成品，后端只做订单关联 + 跨帧去重 + 时间链。
> - **工作台主颗粒度改为「申请号 `crmApplyNo`」**（不是 COD 订单号），见 [`工作台页面功能说明.md`](工作台页面功能说明.md) 与 [`采集客户会话识别规则.md`](采集客户会话识别规则.md)。
